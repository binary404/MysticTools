...
import binary404.mystictools.common.util.Counter;
import binary404.mystictools.common.util.Tuple;
        ...
        if (event.phase == TickEvent.Phase.END) {
            ...
            synchronized (lock) {
                ...
                while (iterator.hasNext()) {
                    ...
                    r.getValue().decrement();
                    if (r.getValue().getValue() <= 0) {
                        r.getKey().run();
                        iterator.remove();
                    }
                }
                ...
                for (Tuple<Runnable, Integer> wait : waiting) {
                    queue.addLast(new Tuple<>(wait.getKey(), new Counter(wait.getValue())));
                }
            }
            waiting.clear();
        }
